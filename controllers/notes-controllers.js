...
const updateNote = async (req, res, next) => {
  ...
  let updatedNote;
  try {
    updatedNote = await Note.findOne({ _id: noteId, userId: userId });
    if (!updatedNote) {
...
const deleteNote = async (req, res, next) => {
  const { uid, nid } = req.params; // Note ID, User ID

  let deletedNote;
  try {
    // First, find the note to delete
    deletedNote = await Note.findOne({ _id: nid, userId: uid });
    if (!deletedNote) {
      const error = new HttpError("Could not find note for provided id", 404);
      return next(error);
    }
  ...
  try {
    ...
    await Note.findOneAndDelete({ _id: nid, userId: uid }, { session: sess });

    // Find the user
    const user = await User.findOne({ _id: uid }).session(sess);
    if (!user) {
...
const shareNote = async (req, res, next) => {
  try {
    ...
    const note = await Note.findOne({ _id: nid, userId: uid });
    if (!note) {
      return res
        .status(404)
